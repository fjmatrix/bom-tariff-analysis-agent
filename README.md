...
and write a short decision brief and comparison CSV.
...
- `calculate_duty_scenarios()`: deterministic HTS rate selection, arithmetic,
  rankings, and country comparisons.
...
invalidates that selection. The simplified prompt invalidates legacy selections.
...
Results are written to `trade_countries.csv` with the exact period, HTS code,
rank, ISO-2 country, country name, and customs value. The official
...
failure is attached to the calculation's `trade_data.errors`; affected parts
still receive a current-origin calculation but no discovered alternatives.
...
Amounts are EUR per finished product. Parts are assumed imported separately,
using unchanged BOM purchase values as the duty base. Duty is rounded to cents
per part before totals. Savings compare each scenario with the part's current
origin. Best savings select at most one alternative per part.
...
```text
level,component_reference,component_name,component_quantity,parent_bom_reference,has_child_bom,unit_price_eur,country_of_origin
```
...
Outputs are `brief.md`, `scenarios.csv`, `trade_countries.csv`, `components.csv`,
`classified.csv`, and `selection_cache.json`. Tool calls and results print in the terminal.
The scenario CSV columns remain `reference,country,duty_eur,savings_eur`.
...
excluded from totals and listed as unresolved. An unsupported alternative rate
excludes only that country scenario. The loaded HTS tree covers heading 7318,
...
| `src/agent/loop.py`, `prompts.py`, `cache.py` | Structured classification and caching |
...
| `src/duty/scenarios.py` | Duty arithmetic, savings, rankings, and CSV |
...
.venv/bin/python -m pytest tests/test_agent_loop.py tests/test_dataweb.py \
  tests/test_duty_scenarios.py tests/test_run.py tests/test_hts_index.py \
  tests/test_hts_render.py -q
```
